...
import 'package:account_connect/models/all_transactions.dart';
import 'package:account_connect/models/update_user.dart';
import 'package:account_connect/models/user_info.dart';
...
class ApiService {
  ...
  Future<UpdateUser?> updateUser(userid, deposit_rate,withdraw_rate) async {
   ...
   Future<UserRegistration?> registerUser(String phone) async {
    final Map<String, dynamic> userData = {
      "sender":phone
    };
    var client = http.Client();
    ...
    final response = await client
        .patch(Uri.parse('${api_url}create_user_account'),
            headers: {
              'Content-Type': 'application/json',
            },
            body: json.encode(userData))
    ...
    if (response.statusCode == 200) {
      return userRegistrationFromJson(response.body);
    } else {
      return null;
    }
  }

    Future<UserInfo?> getUserInfo(String phone) async {
    
    var client = http.Client();

    final response = await client
        .get(Uri.parse('${api_url}get_user_info/${phone}'),
            headers: {
              'Content-Type': 'application/json',
            })
        .catchError(
